...
let mk_info_tbl buffer sections =
  ...
  AddrTbl.add h (fst startup_code)
    (["OCaml" ; "startup" ; "code"], Info.mk ~v:(fst startup_code) ~size:startup_code_size Info.Module);
  AddrTbl.add h (fst startup_data)
    (["OCaml" ; "startup" ; "data"], Info.mk ~v:(fst startup_data) ~size:startup_data_size Info.Module);
  AddrTbl.add h (fst system_code)
    (["OCaml" ; "system"], Info.mk ~v:(fst system_code) ~size:system_size Info.Module);
  h
